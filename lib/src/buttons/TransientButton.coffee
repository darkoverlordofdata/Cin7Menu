class TransientButton
  ...
  _init:(appsMenuButton, pathOrCommand) ->
    ...
    try
      ...
      @icon = new St.Icon({gicon: themedIcon, icon_size: APPLICATION_ICON_SIZE, icon_type: St.IconType.FULLCOLOR })
      @actor.set_style_class_name('menu-application-button')
    catch e
      @handler = null
      iconName = if @isPath then 'gnome-folder' else 'unknown'
      @icon = new St.Icon(icon_name: iconName, icon_size: APPLICATION_ICON_SIZE, icon_type: St.IconType.FULLCOLOR)
      # @todo Would be nice to indicate we don't have a handler for this file.
      @actor.set_style_class_name('menu-application-button')

    @addActor(@icon)

    @label = new St.Label(text: displayPath, style_class: 'menu-application-button-label')
    @addActor(@label)
  ...
  activate:(event) ->
    if @handler isnt null
      ...
      try
        Util.spawn(['gvfs-open', @file.get_uri()])
      catch e
