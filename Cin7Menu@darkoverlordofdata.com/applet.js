...
ApplicationContextMenuItem = (function() {
  ...
  ApplicationContextMenuItem.prototype.activate = function(event) {
    var desktopFiles, destFile, e, enabled_applets, error, file, new_applet_id, settings, winListApplet;
    switch (this._action) {
      case "add_to_panel":
        winListApplet = false;
        try {
...
ApplicationButton = (function(superClass) {
  ...
  function ApplicationButton(appsMenuButton, app) {
    this._init(appsMenuButton, app, true);
...
MyApplet = (function() {
  ...
  MyApplet.prototype._addEnterEvent = function(button, callback) {
    var _callback;
    _callback = Lang.bind(this, function() {
      ...
      if (this._previousTreeSelectedActor && this._activeContainer !== this.categoriesBox && parent !== this._activeContainer && button !== this._previousTreeSelectedActor) {
      ...
      _maybePreviousActor = this._activeActor;
  ...
  MyApplet.prototype._refreshApps = function() {
    var bookmarks, button, categoryButton, devices, dir, i, id, iter, nextType, place, places, root, tree, trees;
    ...
    if (this.showRecent) {
      ...
      while (id < MAX_RECENT_FILES && id < this.RecentManager._infosByTimestamp.length) {
        button = new RecentButton(this, this.RecentManager._infosByTimestamp[id]);
        this._addEnterEvent(button, Lang.bind(this, function() {
          this._clearPrevAppSelection(button.actor);
          button.actor.style_class = "menu-application-button-selected";
          this._scrollToButton(button);
  ...
  MyApplet.prototype._refreshFavs = function() {
    var app, appSys, button, favoritesBox, i, j, launchers;
    ...
    while (i < launchers.length) {
      ...
      if (app) {
        button = new FavoritesButton(this, app, launchers.length, this.favorite_icon_size);
        ...
        button.actor.connect("enter-event", Lang.bind(this, function() {
          ...
          if (button.app.get_description()) {
            this.selectedAppDescription.set_text(button.app.get_description());
          } else {
            this.selectedAppDescription.set_text("");
          }
        }));
  ...
  MyApplet.prototype._loadCategory = function(dir, top_dir) {
    var app, applicationButton, dupe, entry, i, iter, nextType, subdir;
    iter = dir.iter();
    dupe = false;
    nextType = void 0;
    if (!top_dir) {
