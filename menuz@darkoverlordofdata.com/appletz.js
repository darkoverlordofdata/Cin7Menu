...
VisibleChildIterator.prototype = {
    ...
    reloadVisible: function() {
        this.array = this.container.get_focus_chain()
            .filter(function(x){!(x._delegate instanceof PopupMenu.PopupSeparatorMenuItem)});
    },
...
RecentButton.prototype = {
    ...
    toggleMenu: function() {
        if (!this.menu.isOpen){
            ...
            var file = Gio.File.new_for_uri(this.file.uri);

            var default_info = Gio.AppInfo.get_default_for_type(this.file.mimeType, !this.hasLocalPath(file));
            ...
            var infos = Gio.AppInfo.get_all_for_type(this.file.mimeType)
...
GenericButton.prototype = {
    ...
    _init: function(label, icon, reactive, callback) {
        ...
        if (icon != null) {
            var icon_actor = new St.Icon({ icon_name: icon, icon_type: St.IconType.FULLCOLOR, icon_size: APPLICATION_ICON_SIZE});
...
SystemButton.prototype = {
    ...
    _init: function(appsMenuButton, icon, nbFavorites, name, desc) {
        ...
        var iconObj = new St.Icon({icon_name: icon, icon_size: icon_size, icon_type: St.IconType.FULLCOLOR});
...
MyApplet.prototype = {
    ...
    _recalc_height: function() {
        var scrollBoxHeight = (this.leftBox.get_allocation_box().y2-this.leftBox.get_allocation_box().y1) -
    ...
    _onOpenStateChanged: function(menu, open) {
        if (open) {
            ...
            var n = Math.min(this._applicationsButtons.length,
                             INITIAL_BUTTON_LOAD);
            for (var i = 0; i < n; i++) {
    ...
    makeVectorBox: function(actor) {
        ...
        var [mx, my, mask] = global.get_pointer();
        var [bx, by] = this.categoriesApplicationsBox.actor.get_transformed_position();
        var [bw, bh] = this.categoriesApplicationsBox.actor.get_transformed_size();
        var [aw, ah] = actor.get_transformed_size();
        var [ax, ay] = actor.get_transformed_position();
        var [appbox_x, appbox_y] = this.applicationsBox.get_transformed_position();
    ...
    _refreshPlaces : function() {
        ...
        for (var i = 0; i < this._categoryButtons.length; i++) {
        ...
        if (this.showPlaces) {
            ...
            this.placesButton.actor.connect('leave-event', Lang.bind(this, function () {
                if (this._previousTreeSelectedActor === null) {
                    ...
                    var nextIdx = this.catBoxIter.getVisibleIndex(this.placesButton.actor);
    ...
    _refreshRecent : function() {
        ...
        if (this.privacy_settings.get_boolean(REMEMBER_RECENT_KEY)) {
            ...
            if (this.RecentManager._infosByTimestamp.length > 0) {
                ...
                var button = new GenericButton(_("No recent documents"), null, false, null);
                this._recentButtons.push(button);
                this.applicationsBox.add_actor(button.actor);
            }
    ...
    _refreshFavs : function() {
        ...
        for ( var i = 0; i < launchers.length; ++i ) {
            ...
            if (app) {
                var button = new FavoritesButton(this, app, launchers.length + 3); // + 3 because we're adding 3 system buttons at the bottom
        ...
        //Lock screen
        var button = new SystemButton(this, "system-lock-screen", launchers.length + 3,
                                      _("Lock screen"),
                                      _("Lock the screen"));
        ...
        button.activate = Lang.bind(this, function() {
            ...
            var screensaver_settings = new Gio.Settings({ schema_id: "org.cinnamon.desktop.screensaver" });
        ...
        //Logout button
        var button = new SystemButton(this, "system-log-out", launchers.length + 3,
                                      _("Logout"),
                                      _("Leave the session"));
        ...
        //Shutdown button
        var button = new SystemButton(this, "system-shutdown", launchers.length + 3,
                                      _("Quit"),
    ...
    _display : function() {
        ...
        var leftPane = new St.BoxLayout({ vertical: true });
        ...
        var rightPane = new St.BoxLayout({ vertical: true });
    ...
    _clearAllSelections: function(hide_apps) {
        ...
        var actors = this.favoritesBox.get_children();
        for (var i=0; i<actors.length; i++){
            var actor = actors[i];
    ...
    _resize_actor_iter: function(actor) {
        var [min, nat] = actor.get_preferred_width(-1.0);
       ...
       // in the search entry.
        ...
        SearchProviderManager.launch_all(pattern, Lang.bind(this, function(provider, results){
            ...
            for (var i in results){
                ...
                {
                    var button = new SearchProviderResultButton(this, provider, results[i]);
                    button.actor.connect('leave-event', Lang.bind(this, this._appLeaveEvent, button));
                    this._addEnterEvent(button, Lang.bind(this, this._appEnterEvent, button));
